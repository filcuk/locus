...
Self-hosted LAN URLs are often `http://…` (no TLS). Android blocks cleartext by default; the app config plugin `withAndroidCleartextTraffic` sets `android:usesCleartextTraffic` in `apps/app/app.config.ts`. Changing that flag requires a **new native build** (`prebuild` + `run:android` / reinstall), not just a Metro reload.

Two common ways to point the in-app server URL at the maintainer API on port **8000**:
