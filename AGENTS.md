...
`.cursor/rules/git-workflow.mdc` is the enforced version of points 5–7, with the `gh stack` commands, ready-for-review (not draft) rule, and the Bugbot ban. `.cursor/rules/task-board.mdc` is the enforced version of point 8's board semantics; `.cursor/rules/planka-notifications.mdc` owns the notification check.
