...
| Tagging a user in a comment | Anyone who can comment may @mention. If the mentioned user lacks access, the mention creates an **access request** for the owner to approve (does not grant access by itself). A mention may reveal that the resource exists; that side effect is accepted. (Option B; chosen over owner-only tagging.) |
